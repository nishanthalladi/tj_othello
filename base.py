...
def maximize_beginning(state, depth, a, b, token):
    p = possibleMoves(state, token)
    if depth == 0 or not p:
        return eval_beginning(state, TOKEN)
    val = float("-inf")
    for child in p:
...
def minimize_endgame(state, depth, a, b, token):
    p = possibleMoves(state, token)
    if depth == 0 or not p:
        return eval_endgame(state, TOKEN) + eval_weight_mat(state, TOKEN, weight_matrix_end)
    val = float("inf")
    for child in p:
